...
import {STRING, ARRAY, HASH_MAP} from "./constants.js"
import {Stream} from "./collections.js"
...
export default class Virtual {
    ...
    _end() {
        ...
        this._setValue(this._syncValue());
    }
    ...
    _syncValue() {
        ...
        if (reducers.hasOwnProperty(this._type)) {
            const reduce = reducers[this._type];

            return reduce(this._stream.ledger);    
        } else {
            if (this._value === undefined) {
                throw new Error("Virtual value not yet resolved");
            } else {
                return this._value;
            }
        }
